...
exports.brandsWithProducts = async (req, res, next) => {
    try {
        ...
        const query = `SELECT b.brand_name, p."brandId",p.id AS productId,p.name AS productName FROM "Brands" b INNER JOIN "Products" p ON b.id = p."brandId"::integer ORDER BY b.id;`;
...
exports.categoriesWithProducts = async (req, res, next) => {
    try {
        ...
        const query = `SELECT c."Category_name",c.id as category_id,p.name as productName,p.id as productId FROM "Categories" c INNER JOIN "Products" p ON c.id = p."categoryId"::integer ORDER BY c."Category_name",p.id;`;
        const result = await client.query(query);
        if (result.rowCount < 1) {
